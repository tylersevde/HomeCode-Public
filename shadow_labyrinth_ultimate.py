...
# ------------------------------------------------------------
# Shadow Labyrinth Ultimate
# Deluxe gameplay version with procedural textures, lighting,
# particles, screen shake, trails, fog, and impact effects.
# ------------------------------------------------------------
...
pygame.display.set_caption("Shadow Labyrinth Ultimate")
...
class Assets:
    def __init__(self):
        self.floor_tiles = [self.make_floor_tile(i) for i in range(8)]
        self.wall_tiles = [self.make_wall_tile(i) for i in range(8)]
        self.boss_floor_tile = self.make_boss_floor_tile()
        self.glows = {}
        self.vignette = self.make_vignette()
        self.hud_panel = self.make_hud_panel(360, 86)
        self.big_panel = self.make_hud_panel(500, 54)

    def make_floor_tile(self, seed):
        rng = random.Random(1000 + seed)
        surf = pygame.Surface((TILE, TILE)).convert()
        surf.fill(FLOOR if seed % 2 == 0 else FLOOR_ALT)

        for _ in range(100):
            x = rng.randrange(TILE)
            y = rng.randrange(TILE)
            shade = rng.randint(-12, 16)
            base = surf.get_at((x, y))
            color = (
                clamp(base.r + shade, 0, 255),
                clamp(base.g + shade, 0, 255),
                clamp(base.b + shade, 0, 255),
            )
        ...
        for _ in range(7):
            ...
            w = rng.randint(8, 18)
            h = rng.randint(6, 14)
            color = (rng.randint(24, 40), rng.randint(24, 40), rng.randint(35, 52))
            pygame.draw.ellipse(surf, color, (x - w // 2, y - h // 2, w, h), 1)
        ...
        for _ in range(5):
            ...
            for _ in range(4):
                points.append((x, y))
                x = clamp(x + rng.randint(-7, 7), 1, TILE - 2)
                y = clamp(y + rng.randint(-7, 7), 1, TILE - 2)
            pygame.draw.lines(surf, (46, 48, 58), False, points, 1)

        pygame.draw.line(surf, (44, 44, 54), (0, TILE - 1), (TILE, TILE - 1))
        pygame.draw.line(surf, (44, 44, 54), (TILE - 1, 0), (TILE - 1, TILE))
        return surf

    def make_wall_tile(self, seed):
        ...
        for y in range(0, TILE, 12):
            ...
            pygame.draw.line(surf, (54, 57, 72), (0, y), (TILE, y), 2)
            for x in range(offset, TILE, 16):
                pygame.draw.line(surf, (54, 57, 72), (x, y), (x, min(TILE, y + 12)), 2)
        ...
        for _ in range(12):
            x = rng.randrange(2, TILE - 6)
            y = rng.randrange(2, TILE - 6)
            pygame.draw.rect(surf, (86, 90, 112), (x, y, rng.randint(2, 4), rng.randint(2, 4)))

        pygame.draw.rect(surf, WALL_EDGE, (0, 0, TILE, TILE), 2)
        pygame.draw.line(surf, (125, 130, 155), (0, 0), (TILE, 0), 2)
        pygame.draw.line(surf, (125, 130, 155), (0, 0), (0, TILE), 2)
        return surf

    def make_boss_floor_tile(self):
        surf = pygame.Surface((TILE, TILE)).convert()
        surf.fill((36, 18, 24))
        for y in range(TILE):
            ratio = y / max(1, TILE - 1)
            c = (int(36 + ratio * 18), int(16 + ratio * 6), int(24 + ratio * 8))
            pygame.draw.line(surf, c, (0, y), (TILE, y))
        for i in range(0, TILE, 8):
            pygame.draw.line(surf, (55, 25, 30), (i, 0), (i - 8, TILE), 1)
        pygame.draw.rect(surf, (90, 40, 50), (0, 0, TILE, TILE), 2)
        return surf

    def make_hud_panel(self, w, h):
        surf = pygame.Surface((w, h), pygame.SRCALPHA)
        pygame.draw.rect(surf, (14, 14, 20, 210), (0, 0, w, h), border_radius=14)
        pygame.draw.rect(surf, (58, 62, 84, 230), (0, 0, w, h), 2, border_radius=14)
        for y in range(h):
            a = int(22 * (1 - y / max(1, h - 1)))
            pygame.draw.line(surf, (255, 255, 255, a), (10, y), (w - 10, y))
        return surf
    ...
    def make_vignette(self):
        ...
        for y in range(SCREEN_H):
            for x in range(SCREEN_W):
                ...
                alpha = int(clamp((dist - 0.35) * 240, 0, 120))
                surf.set_at((x, y), (0, 0, 0, alpha))
        return surf

    def glow(self, radius, color=(255, 255, 255), strength=180):
        key = (radius, color, strength)
        if key in self.glows:
        ...
        for r in range(radius, 0, -1):
            ...
            pygame.draw.circle(surf, col, (center, center), r)
        self.glows[key] = surf
        return surf
...
class Camera:
    ...
    def update(self, target, shake=(0, 0)):
        self.x = clamp(target.x - SCREEN_W // 2 + shake[0], 0, WORLD_W - SCREEN_W)
        self.y = clamp(target.y - SCREEN_H // 2 + shake[1], 0, WORLD_H - SCREEN_H)
        self.shake = Vector2(shake)
...
class FloatingText:
    ...
    def draw(self, surf, camera):
        ...
        shadow = SMALL_FONT.render(self.text, True, (0, 0, 0))
        shadow.set_alpha(alpha)
        surf.blit(shadow, (pos[0] - txt.get_width() // 2 + 2, pos[1] + 2))
        surf.blit(txt, (pos[0] - txt.get_width() // 2, pos[1]))


class Particle:
    def __init__(self, x, y, vel, color, radius=4, life=0.5, drag=0.93, gravity=0.0, glow=None, fade=True):
    ...
    def draw(self, surf, camera):
        if self.life <= 0:
            return
        alpha = 255 if not self.fade else int(255 * max(0, self.life / self.max_life))
        px, py = camera.apply_point(self.pos)
        if self.glow:
            glow = ASSETS.glow(self.glow, self.color, strength=min(220, alpha))
            surf.blit(glow, (px - self.glow, py - self.glow), special_flags=pygame.BLEND_ADD)
        pygame.draw.circle(surf, (*self.color, alpha), (px, py), max(1, int(self.radius)))


class SlashEffect:
    def __init__(self, pos, facing, color=(200, 235, 255)):
    ...
    def draw(self, surf, camera):
        ...
        width = 4 + int(progress * 4)
        sweep = 0.42 + progress * 0.12
        alpha = int(200 * (self.life / self.max_life))
        px, py = camera.apply_point(self.pos)
        temp = pygame.Surface((180, 180), pygame.SRCALPHA)
        rect = pygame.Rect(20, 20, 140, 140)
        pygame.draw.arc(temp, (*self.color, alpha), rect, self.angle - sweep, self.angle + sweep, width)
        pygame.draw.arc(temp, (*WHITE, alpha // 2), rect.inflate(-12, -12), self.angle - sweep * 0.8, self.angle + sweep * 0.8, 2)
        surf.blit(temp, (px - 90, py - 90))


class AmbientFog:
    def __init__(self):
        self.x = random.uniform(-200, SCREEN_W + 200)
        self.y = random.uniform(-100, SCREEN_H + 100)
        self.speed = random.uniform(8, 24)
        self.radius = random.randint(90, 180)
        self.alpha = random.randint(12, 26)
        self.drift = random.uniform(-10, 10)

    def update(self, dt):
        self.x += self.speed * dt
        self.y += self.drift * dt
        if self.x - self.radius > SCREEN_W + 120:
            self.x = -self.radius - random.randint(20, 120)
            self.y = random.uniform(-80, SCREEN_H + 80)

    def draw(self, surf):
        glow = ASSETS.glow(self.radius, (90, 70, 120), self.alpha)
        surf.blit(glow, (int(self.x - self.radius), int(self.y - self.radius)), special_flags=pygame.BLEND_ADD)
...
class Pickup:
    def __init__(self, x, y, kind, amount=1):
        ...
        self.sparkle_cd = random.uniform(0.0, 0.8)

    def update(self, dt, game=None):
        self.bob += dt * 3.2
        self.sparkle_cd -= dt
        if game and self.sparkle_cd <= 0:
            self.sparkle_cd = random.uniform(0.18, 0.42)
            color = LOOT if self.kind == "loot" else FLASK if self.kind == "flask" else KEY
            ang = random.uniform(0, math.tau)
            p = self.pos + angle_to_vec(ang) * random.uniform(3, 10)
            game.particles.append(Particle(p.x, p.y, angle_to_vec(ang) * random.uniform(6, 18), color, radius=2, life=0.35, drag=0.84, glow=8))

    def collect(self, player, texts, game=None):
        if self.kind == "loot":
        ...
        if game:
            ...
            for _ in range(12):
                ...
                game.particles.append(Particle(self.pos.x, self.pos.y, angle_to_vec(ang) * spd, color, radius=3, life=0.45, drag=0.90, glow=10))

    def draw(self, surf, camera):
        px, py = camera.apply_point(self.pos)
        py += int(math.sin(self.bob) * 4)
        glow_color = LOOT if self.kind == "loot" else FLASK if self.kind == "flask" else KEY
        surf.blit(ASSETS.glow(18, glow_color, 70), (px - 18, py - 18), special_flags=pygame.BLEND_ADD)

        if self.kind == "loot":
            points = [(px, py - 10), (px + 8, py), (px, py + 10), (px - 8, py)]
            pygame.draw.polygon(surf, LOOT, points)
            pygame.draw.polygon(surf, WHITE, points, 2)
            pygame.draw.line(surf, (255, 245, 160), (px, py - 7), (px, py + 7), 2)
        elif self.kind == "flask":
...
class Projectile:
    ...
    def update(self, dt, level, player, texts, game=None):
        self.pos += self.vel * dt
        self.trail_timer -= dt
        if game and self.trail_timer <= 0:
            self.trail_timer = 0.025
            game.particles.append(Particle(self.pos.x, self.pos.y, -self.vel * 0.05, self.color, radius=max(2, self.radius - 3), life=0.22, drag=0.8, glow=12))
        ...
        if level.circle_hits_blocker(self.pos.x, self.pos.y, self.radius):
            self.alive = False
            if game:
                game.spawn_impact(self.pos, self.color, 10, 50, 140, shake=0)
            return
        ...
        if self.owner == "enemy" and (self.pos - player.pos).length() < self.radius + player.radius:
            ...
            if result == "blocked":
                ...
                if game:
                    game.spawn_block_burst(player.pos, self.pos)
            elif result == "hit":
                texts.append(FloatingText(player.pos.x, player.pos.y - 26, f"-{self.damage}", RED))
                if game:
                    game.spawn_blood(player.pos, self.pos)
                    game.add_shake(5, 0.18)
                    game.flash(42)

    def draw(self, surf, camera):
        pos = camera.apply_point(self.pos)
        surf.blit(ASSETS.glow(self.radius * 2, self.color, 92), (pos[0] - self.radius * 2, pos[1] - self.radius * 2), special_flags=pygame.BLEND_ADD)
...
class Door:
    ...
    def draw(self, surf, camera, time_now):
        if not self.locked:
            return
        self.pulse = time_now * 2.0 + self.label
        rect = self.rect.move(-camera.x, -camera.y)
        glow_size = 26 + int((math.sin(self.pulse) * 0.5 + 0.5) * 4)
        surf.blit(ASSETS.glow(glow_size, DOOR, 48), (rect.centerx - glow_size, rect.centery - glow_size), special_flags=pygame.BLEND_ADD)
        pygame.draw.rect(surf, (30, 50, 58), rect, border_radius=4)
        pygame.draw.rect(surf, DOOR, rect, 3, border_radius=4)
...
class Player:
    ...
    def update(self, dt, level, game=None):
        ...
        if moved and game:
            self.footstep -= dt
            if self.footstep <= 0:
                self.footstep = 0.12 if self.shielding else 0.08
                dust_dir = -move * random.uniform(10, 40)
                game.particles.append(Particle(self.pos.x, self.pos.y + 12, dust_dir, (110, 110, 125), radius=3, life=0.25, drag=0.78))
    ...
    def try_use_flask(self, texts, game=None):
        ...
        if game:
            for _ in range(16):
                ...
                game.particles.append(Particle(self.pos.x, self.pos.y, angle_to_vec(ang) * spd, FLASK, radius=3, life=0.55, drag=0.9, glow=10))
            game.flash(18)
        return True
    ...
    def draw(self, surf, camera):
        pos = camera.apply_point(self.pos)
        shadow = pygame.Surface((80, 50), pygame.SRCALPHA)
        pygame.draw.ellipse(shadow, (0, 0, 0, 70), (10, 20, 60, 22))
        surf.blit(shadow, (pos[0] - 40, pos[1] - 18))

        surf.blit(ASSETS.glow(26, PLAYER_BODY, 42), (pos[0] - 26, pos[1] - 26), special_flags=pygame.BLEND_ADD)
        if self.shielding:
            shield_center = self.pos + self.facing * 18
            shield_pos = camera.apply_point(shield_center)
            surf.blit(ASSETS.glow(28, SHIELD, 72), (shield_pos[0] - 28, shield_pos[1] - 28), special_flags=pygame.BLEND_ADD)
            pygame.draw.circle(surf, (32, 88, 112), shield_pos, 18)
            pygame.draw.circle(surf, SHIELD, shield_pos, 18, 3)
            pygame.draw.arc(surf, (200, 245, 255), pygame.Rect(shield_pos[0] - 14, shield_pos[1] - 14, 28, 28), -0.7, 1.6, 2)

        body_color = PLAYER_BODY if self.invuln <= 0 else (170, 220, 255)
        cloak_color = (34, 45, 62)
        back = self.pos - self.facing * 8
        pygame.draw.circle(surf, cloak_color, camera.apply_point(back), self.radius + 2)
        pygame.draw.circle(surf, body_color, pos, self.radius)
        pygame.draw.circle(surf, (72, 130, 190), pos, self.radius, 2)
        eye = self.pos + self.facing * 7
        eye_pos = camera.apply_point(eye)
        pygame.draw.circle(surf, PLAYER_TRIM, eye_pos, 3)

        shoulder_left = self.pos + self.facing.rotate(90) * 8
        shoulder_right = self.pos + self.facing.rotate(-90) * 8
        pygame.draw.line(surf, (180, 210, 240), camera.apply_point(shoulder_left), camera.apply_point(shoulder_right), 2)

        if self.attack_timer > 0:
            sword_tip = self.pos + self.facing * 44
            left = self.pos + self.facing.rotate(30) * 22
            right = self.pos + self.facing.rotate(-30) * 22
            pygame.draw.polygon(
                surf,
                SWORD,
...
class Enemy:
    def __init__(self, x, y, kind):
        ...
        if kind == "stalker":
            ...
            self.name = "Shade"
            self.loot_drop = 5
            self.glow_color = (160, 70, 180)

    def update(self, dt, level, player, projectiles, texts, game=None):
        if not self.alive:
            return

        self.anim += dt * (5.2 if self.kind == "stalker" else 3.0)
        self.attack_cd = max(0, self.attack_cd - dt)
        ...
        if self.kind == "cultist":
            if dist < self.detect:
                ...
                if dist < 360 and self.ranged_cd <= 0:
                    ...
                    if game:
                        game.spawn_cast_burst(self.pos, self.glow_color)
            else:
        ...
        if dist < melee_range and self.attack_cd <= 0 and self.kind != "cultist":
            ...
            if result == "blocked":
                ...
                if game:
                    game.spawn_blood(player.pos, self.pos)
                    game.add_shake(4 if self.kind != "brute" else 6, 0.18)
                    game.flash(35 if self.kind != "brute" else 50)
    ...
    def draw(self, surf, camera):
        if not self.alive:
            return
        pos = camera.apply_point(self.pos)
        bob = math.sin(self.anim) * 1.5
        draw_pos = (pos[0], int(pos[1] + bob))
        shadow = pygame.Surface((self.radius * 4, self.radius * 2), pygame.SRCALPHA)
        pygame.draw.ellipse(shadow, (0, 0, 0, 66), (0, self.radius // 2, self.radius * 4, self.radius))
        surf.blit(shadow, (draw_pos[0] - self.radius * 2, draw_pos[1] - self.radius // 2))
        glow_radius = self.radius + (10 if self.kind != "brute" else 14)
        surf.blit(ASSETS.glow(glow_radius, self.glow_color, 54), (draw_pos[0] - glow_radius, draw_pos[1] - glow_radius), special_flags=pygame.BLEND_ADD)
        pygame.draw.circle(surf, self.color, draw_pos, self.radius)
        pygame.draw.circle(surf, (15, 10, 20), draw_pos, self.radius, 2)
        if self.kind == "brute":
            pygame.draw.rect(surf, self.eye, (draw_pos[0] - 8, draw_pos[1] - 4, 16, 4), border_radius=2)
            pygame.draw.rect(surf, (130, 55, 65), (draw_pos[0] - 10, draw_pos[1] + 7, 20, 4), border_radius=2)
        else:
            pygame.draw.circle(surf, self.eye, (draw_pos[0] - 5, draw_pos[1] - 2), 2)
            pygame.draw.circle(surf, self.eye, (draw_pos[0] + 5, draw_pos[1] - 2), 2)
            pygame.draw.arc(surf, (110, 50, 80), pygame.Rect(draw_pos[0] - 8, draw_pos[1] + 2, 16, 8), 0, math.pi, 1)

        hp_ratio = self.hp / self.max_hp
        bar_rect = pygame.Rect(draw_pos[0] - 18, draw_pos[1] - self.radius - 12, 36, 5)
        pygame.draw.rect(surf, (40, 20, 20), bar_rect, border_radius=3)
        pygame.draw.rect(surf, RED, (bar_rect.x, bar_rect.y, int(bar_rect.w * hp_ratio), bar_rect.h), border_radius=3)
...
class Boss:
    ...
    def update(self, dt, level, player, projectiles, texts, game=None):
        ...
        if self.charge_time > 0:
            ...
            if game and random.random() < 0.45:
                game.particles.append(Particle(self.pos.x, self.pos.y, Vector2(random.uniform(-30, 30), random.uniform(-30, 30)), BOSS_CORE, radius=4, life=0.3, drag=0.8, glow=14))
        else:
            move = direction * self.speed * dt
            if self.volley_cd <= 0:
                ...
                if game:
                    game.spawn_cast_burst(self.pos, BOSS_CORE, 28)
                    game.flash(20)
            if self.charge_cd <= 0 and dist > 120:
                ...
                self.charge_cd = 4.0
                if game:
                    game.add_shake(5, 0.32)
                    game.spawn_cast_burst(self.pos, (255, 70, 80), 22)
    ...
    def draw(self, surf, camera):
        if not self.alive:
            return
        pos = camera.apply_point(self.pos)
        pulse = math.sin(self.anim * 1.3) * 0.5 + 0.5
        outer = 62 + int(pulse * 10)
        surf.blit(ASSETS.glow(outer, (180, 40, 50), 56), (pos[0] - outer, pos[1] - outer), special_flags=pygame.BLEND_ADD)
        surf.blit(ASSETS.glow(28, BOSS_CORE, 120), (pos[0] - 28, pos[1] - 28), special_flags=pygame.BLEND_ADD)
        pygame.draw.circle(surf, (70, 20, 30), pos, 52)
        pygame.draw.circle(surf, BOSS, pos, self.radius)
        pygame.draw.circle(surf, (35, 10, 15), pos, self.radius, 3)
        pygame.draw.circle(surf, BOSS_CORE, pos, 12 + int(pulse * 2))
        ...
        eye_offset = 10 + int(pulse)
        pygame.draw.circle(surf, WHITE, (pos[0] - eye_offset, pos[1] - 3), 3)
        pygame.draw.circle(surf, WHITE, (pos[0] + eye_offset, pos[1] - 3), 3)
        pygame.draw.arc(surf, (255, 160, 170), pygame.Rect(pos[0] - 12, pos[1] + 2, 24, 10), 0, math.pi, 2)
...
class Level:
        ...
        for door in self.doors:
            ...
            if circle_rect_collision(player.pos.x, player.pos.y, player.radius + 2, door.rect):
                if player.keys > 0:
                    ...
                    game.spawn_gate_open(door.rect.center)
                    game.flash(16)
    ...
    def tile_variant(self, x, y):
        return hash_xy(x, y) % 8

    def draw(self, surf, time_now=0.0):
        surf.fill(BG)
        ...
        for y in range(start_y, end_y):
            for x in range(start_x, end_x):
                px = x * TILE - self.camera.x
                py = y * TILE - self.camera.y
                in_boss_arena = abs(x - bx) <= 3 and abs(y - by) <= 3
                if self.grid[y][x] == 1:
                    surf.blit(ASSETS.wall_tiles[self.tile_variant(x, y)], (px, py))
                else:
                    tile = ASSETS.boss_floor_tile if in_boss_arena else ASSETS.floor_tiles[self.tile_variant(x, y)]
                    surf.blit(tile, (px, py))
                    if (x + y) % 9 == 0:
                        alpha = 18 + int((math.sin(time_now * 1.4 + x * 0.2 + y * 0.3) * 0.5 + 0.5) * 12)
                        glow = pygame.Surface((TILE, TILE), pygame.SRCALPHA)
                        pygame.draw.circle(glow, (90, 70, 120, alpha), (TILE // 2, TILE // 2), 9)
                        surf.blit(glow, (px, py), special_flags=pygame.BLEND_ADD)
        ...
        for i in range(5):
            pulse = int(24 + 4 * math.sin(time_now * 2.0 + i))
            surf.blit(ASSETS.glow(20 + i * 4, (100, 30, 40), pulse), (arena.centerx - (20 + i * 4), arena.centery - (20 + i * 4)), special_flags=pygame.BLEND_ADD)

        for door in self.doors:
            door.draw(surf, self.camera, time_now)
...
class Game:
    ...
    def reset(self):
        ...
        self.effects = []
        self.particles = []
        self.ambient_fog = [AmbientFog() for _ in range(10)]
        self.state = "title"
        self.enemy_count = 54
        self.message_cd = 0
        self.time = 0.0
        ...
        self.light_surface = pygame.Surface((SCREEN_W, SCREEN_H), pygame.SRCALPHA)
        self.dim_surface = pygame.Surface((SCREEN_W, SCREEN_H), pygame.SRCALPHA)
        self.dim_surface.fill((8, 8, 14, 70))
        self.spawn_enemies()
        self.spawn_pickups()
        self.spawn_boss()

    def add_shake(self, power, duration):
    ...
    def current_shake(self):
        if self.shake_time <= 0 or self.shake_power <= 0:
            return (0, 0)
        return (random.uniform(-self.shake_power, self.shake_power), random.uniform(-self.shake_power, self.shake_power))

    def spawn_impact(self, pos, color, count=10, min_speed=60, max_speed=160, shake=2):
        for _ in range(count):
            ang = random.uniform(0, math.tau)
            spd = random.uniform(min_speed, max_speed)
            self.particles.append(Particle(pos.x, pos.y, angle_to_vec(ang) * spd, color, radius=random.randint(2, 4), life=0.35, drag=0.88, glow=10))
        if shake:
            self.add_shake(shake, 0.1)

    def spawn_blood(self, pos, source):
        base_dir = normalize_or_zero(Vector2(pos) - Vector2(source))
        for _ in range(14):
            ang = math.atan2(base_dir.y, base_dir.x) + random.uniform(-0.8, 0.8)
            spd = random.uniform(40, 170)
            color = random.choice([(220, 65, 70), (180, 45, 55), (120, 20, 32)])
            self.particles.append(Particle(pos.x, pos.y, angle_to_vec(ang) * spd, color, radius=random.randint(2, 4), life=0.45, drag=0.9))

    def spawn_block_burst(self, pos, source):
        dir_vec = normalize_or_zero(Vector2(pos) - Vector2(source))
        for _ in range(12):
            ang = math.atan2(dir_vec.y, dir_vec.x) + random.uniform(-0.9, 0.9)
            spd = random.uniform(40, 190)
            self.particles.append(Particle(pos.x, pos.y, angle_to_vec(ang) * spd, SHIELD, radius=random.randint(2, 4), life=0.32, drag=0.82, glow=12))

    def spawn_cast_burst(self, pos, color, count=16):
        for _ in range(count):
            ang = random.uniform(0, math.tau)
            spd = random.uniform(35, 110)
            self.particles.append(Particle(pos.x, pos.y, angle_to_vec(ang) * spd, color, radius=random.randint(2, 4), life=0.42, drag=0.9, glow=14))

    def spawn_gate_open(self, center):
        cx, cy = center
        for _ in range(16):
            ang = random.uniform(0, math.tau)
            spd = random.uniform(50, 180)
            self.particles.append(Particle(cx, cy, angle_to_vec(ang) * spd, DOOR, radius=random.randint(2, 4), life=0.55, drag=0.9, glow=16))
    ...
    def start_attack(self):
        if self.player.try_attack():
            ...
            arc_pos = self.player.pos + self.player.facing * 24
            self.spawn_impact(arc_pos, (190, 235, 255), count=6, min_speed=30, max_speed=110, shake=0)
            return True
        return False
            ...
            if event.type == pygame.KEYDOWN:
                if self.state == "title":
                    ...
                    if event.key in (pygame.K_f, pygame.K_q):
                        self.player.try_use_flask(self.texts, self)
    ...
    def handle_player_hits(self):
        for enemy in self.enemies:
            if enemy.alive and enemy.last_hit_by_attack != self.player.attack_id and self.player.sword_hits(enemy.pos, enemy.radius):
                ...
                self.spawn_impact(enemy.pos, YELLOW, 12, 60, 180, shake=2)
                if not enemy.alive:
                    self.drop_from_enemy(enemy)
                    self.spawn_impact(enemy.pos, enemy.glow_color, 22, 60, 220, shake=4)
                    self.flash(14)
        ...
        if self.boss and self.boss.alive and self.boss.last_hit_by_attack != self.player.attack_id and self.player.sword_hits(self.boss.pos, self.boss.radius):
            ...
            self.spawn_impact(self.boss.pos, BOSS_CORE, 18, 60, 210, shake=4)
            self.flash(10)

    def handle_pickups(self, dt):
        for pickup in self.pickups:
            pickup.update(dt, self)
            if pickup.alive and (pickup.pos - self.player.pos).length() < pickup.radius + self.player.radius + 2:
                pickup.collect(self.player, self.texts, self)
        self.pickups = [p for p in self.pickups if p.alive]

    def update(self, dt):
        self.time += dt
        self.shake_time = max(0, self.shake_time - dt)
        self.flash_alpha = max(0, self.flash_alpha - 110 * dt)
        if self.shake_time <= 0:
            self.shake_power = 0
        else:
            self.shake_power *= 0.9

        for fog in self.ambient_fog:
            fog.update(dt)
        ...
        self.player.update(dt, self.level, self)
        self.level.handle_doors(self.player, self.texts, self)
        self.level.camera.update(self.player.pos, self.current_shake())
        self.level.update_exploration(self.player)

        for enemy in self.enemies:
            enemy.update(dt, self.level, self.player, self.projectiles, self.texts, self)
        self.handle_player_hits()
        self.enemies = [e for e in self.enemies if e.alive]

        if self.boss and self.boss.alive:
            self.boss.update(dt, self.level, self.player, self.projectiles, self.texts, self)
            if not self.boss.alive:
                self.pickups.append(Pickup(self.boss.pos.x, self.boss.pos.y, "loot", 50))
                self.spawn_impact(self.boss.pos, BOSS_CORE, 42, 80, 300, shake=10)
                self.flash(90)
                self.state = "win"

        for projectile in self.projectiles:
            projectile.update(dt, self.level, self.player, self.texts, self)
        self.projectiles = [p for p in self.projectiles if p.alive]

        self.handle_pickups(dt)
    ...
    def draw_hud(self):
        screen.blit(ASSETS.hud_panel, (12, 12))
        ...
        if self.boss and self.boss.alive:
            screen.blit(ASSETS.big_panel, (SCREEN_W // 2 - 250, 10))
    ...
    def draw_title(self):
        ...
        for fog in self.ambient_fog:
            fog.draw(screen)
        title = BIG_FONT.render("SHADOW LABYRINTH ULTIMATE", True, WHITE)
        ...
        lines = [
            ...
            "Now with textures, glow, particles, fog, and hit effects.",
        ]
        title_y = 70
        lines_y = 170
        prompt_y = SCREEN_H - 40
        screen.blit(title, (SCREEN_W // 2 - title.get_width() // 2, title_y))
        for i, line in enumerate(lines):
            txt = FONT.render(line, True, (185, 185, 205))
            screen.blit(txt, (SCREEN_W // 2 - txt.get_width() // 2, lines_y + i * 32))
        screen.blit(prompt, (SCREEN_W // 2 - prompt.get_width() // 2, prompt_y))
        screen.blit(ASSETS.vignette, (0, 0))
    ...
    def draw_lighting(self):
        ...
        player_screen = self.level.camera.apply_point(self.player.pos)
        self.light_surface.blit(ASSETS.glow(170, (90, 130, 200), 60), (player_screen[0] - 170, player_screen[1] - 170), special_flags=pygame.BLEND_ADD)

        for pickup in self.pickups:
            px, py = self.level.camera.apply_point(pickup.pos)
            color = LOOT if pickup.kind == "loot" else FLASK if pickup.kind == "flask" else KEY
            self.light_surface.blit(ASSETS.glow(40, color, 36), (px - 40, py - 40), special_flags=pygame.BLEND_ADD)

        for projectile in self.projectiles:
            px, py = self.level.camera.apply_point(projectile.pos)
            self.light_surface.blit(ASSETS.glow(36, projectile.color, 42), (px - 36, py - 36), special_flags=pygame.BLEND_ADD)

        for enemy in self.enemies:
            if not enemy.alive:
                continue
            px, py = self.level.camera.apply_point(enemy.pos)
            self.light_surface.blit(ASSETS.glow(enemy.radius + 24, enemy.glow_color, 22), (px - enemy.radius - 24, py - enemy.radius - 24), special_flags=pygame.BLEND_ADD)

        if self.boss and self.boss.alive:
            px, py = self.level.camera.apply_point(self.boss.pos)
            self.light_surface.blit(ASSETS.glow(120, BOSS_CORE, 46), (px - 120, py - 120), special_flags=pygame.BLEND_ADD)

        screen.blit(self.dim_surface, (0, 0))
        screen.blit(self.light_surface, (0, 0), special_flags=pygame.BLEND_ADD)
        screen.blit(ASSETS.vignette, (0, 0))
    ...
    def draw(self):
        ...
        self.level.draw(screen, self.time)
        ...
        for effect in self.effects:
            effect.draw(screen, self.level.camera)
        for particle in self.particles:
            particle.draw(screen, self.level.camera)
        for text in self.texts:
        ...
        if self.flash_alpha > 0:
            ...
            screen.blit(flash, (0, 0), special_flags=pygame.BLEND_ADD)
